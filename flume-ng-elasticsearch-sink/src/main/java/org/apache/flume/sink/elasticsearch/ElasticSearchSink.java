...
//        }
      ...
      logger.error("=ElasticSearchSink=>has error", thr);
      try {
        txn.commit();
        this.sinkCounter.addToEventDrainSuccessCount(batchSize);
        this.counterGroup.incrementAndGet("transaction.success");
      } catch (Exception ex2) {
        logger.error("=ElasticSearchSink.counterGroup.incrementAndGet=> has exception.", ex2);
      }
    } finally {
      try {
        txn.close();
      }catch(Exception exc){
        logger.error("=ElasticSearchSink=>txn.close() has error:",exc);
      }
    }
    return status;
  }
  ...
  public void start() {
    ...
    super.start();
    logger.info("=ElasticSearch=> sink end");
  }
