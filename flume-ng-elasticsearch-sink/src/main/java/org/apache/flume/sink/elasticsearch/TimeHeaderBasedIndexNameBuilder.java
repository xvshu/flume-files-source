 ...
 */
public class TimeHeaderBasedIndexNameBuilder implements
        IndexNameBuilder {
  private static final Logger logger = LoggerFactory.getLogger(TimeHeaderBasedIndexNameBuilder.class);
  ...
  public static String headerKey = "UlogDomain";
  ...
  public String getIndexName(Event event) {
    ...
    if(event.getHeaders().get(headerKey)!=null && event.getHeaders().get(headerKey).trim().length()>0){
      domain=event.getHeaders().get(headerKey).trim();
    }
    domain=domain.toLowerCase();
    return new StringBuilder(realIndexPrefix).append('-').append(domain).append('-')
