...
public class KafkaChannel extends BasicChannelSemantics {
  ...
  */
  ...
  public void stop() {
    try {
      logger.info("=KafkaChannel.stop=> begin");
      ...
      counter.stop();
      super.stop();
      logger.info("=KafkaChannel.stop=>Kafka channel {} stopped. Metrics: {}", getName(),
              counter);
    }catch (Exception ex){
      logger.error("=KafkaChannel.start=>error:",ex);
    }
    logger.info("=KafkaChannel.stop=> end");
  }
  ...
  public void configure(Context ctx) {
    try {
      ...
      logger.error("==KafkaChannel.configure==>",exe);
    }
  }
  ...
  private synchronized ConsumerAndRecords createConsumerAndRecords() {
    try {
      ...
      logger.error("=kafkachannel.createConsumerAndRecords=>",e);
      throw new FlumeException("Unable to connect to Kafka", e);
    }
  }

  private void migrateOffsets() {
    logger.info("=KafkaChannel.migrateOffsets=> begin ");
    ZkUtils zkUtils = ZkUtils.apply(zookeeperConnect, ZK_SESSION_TIMEOUT, ZK_CONNECTION_TIMEOUT,
        JaasUtils.isZkSecurityEnabled());
    logger.info("=KafkaChannel.migrateOffsets=> crete KafkaConsumer consumerProps: "+ JSON.toString(consumerProps));
    ...
    try {
      ...
      if (!kafkaOffsets.isEmpty()) {
        ...
        logger.info("Offsets found: {}", kafkaOffsets);
        logger.info("=KafkaChannel.migrateOffsets=> end !kafkaOffsets.isEmpty()");
        return;
      }
      ...
      if (zookeeperOffsets.isEmpty()) {
        ...
        logger.info("=KafkaChannel.migrateOffsets=> end zookeeperOffsets.isEmpty() ");
        return;
      }
      ...
      logger.error("=KafkaChannel.migrateOffsets=> error :",th);
    }finally {
      zkUtils.close();
      consumer.close();
    }
  ...
  private Map<TopicPartition, OffsetAndMetadata> getKafkaOffsets(
    ...
    logger.info("=KafkaChannel.getKafkaOffsets=> end offsets: "+JSON.toString(offsets));
    return offsets;
  }
  ...
  private Map<TopicPartition, OffsetAndMetadata> getZookeeperOffsets(ZkUtils client) {
    ...
    logger.info("=KafkaChannel.getZookeeperOffsets=> end offsets: "+JSON.toString(offsets));
    return offsets;
  }

  private void decommissionConsumerAndRecords(ConsumerAndRecords c) {
    logger.info("=KafkaChannel.decommissionConsumerAndRecords=> c.consumer.close();");
    try {
      c.consumer.close();
    }catch (Throwable th){
      logger.error("=KafkaChannel.decommissionConsumerAndRecords=> has error :",th);
    }
  }
  ...
  void registerThread() {
    try {
      ...
      logger.error("=KafkaChannel.registerThread=>",e);
      e.printStackTrace();
    }
  }
  ...
  private class KafkaTransaction extends BasicTransactionSemantics {
    ...
    protected void doBegin() throws InterruptedException {
      logger.info("=KafkaTransaction=> dobegin() begin");
      rebalanceFlag.set(false);
      logger.info("=KafkaTransaction=> dobegin() end");
    }

    @Override
    protected void doPut(Event event) throws InterruptedException {
      logger.info("=KafkaTransaction=> doput() begin");
      try {
        ...
        logger.error("=KafkaTransaction=>doPut error",ex);
      }
      logger.info("=KafkaTransaction=> doput() end");
    }

    @SuppressWarnings("unchecked")
    @Override
    protected Event doTake() throws InterruptedException {
      logger.info("=KafkaTransaction=>doTake begin");
      Event e=null;
      try {
        ...
        try {
          ...
          logger.error("Error while shutting down consumer", ex);
        }
        if (!events.isPresent()) {
        ...
        if (rebalanceFlag.get()) {
          ...
          logger.info("=KafkaTransaction=>doTake end return null rebalanceFlag.get(): "+rebalanceFlag.get());
          return null;
        }
        if (!consumerAndRecords.get().failedEvents.isEmpty()) {
          ...
          try {
            ...
            if (consumerAndRecords.get().recordIterator.hasNext()) {
              ...
              logger.info("=KafkaTransaction=>doTake end return null consumerAndRecords.get().recordIterator.hasNext() ");
              return null;
            }
          } catch (Exception ex) {
            logger.error("=KafkaTransaction=>Error while getting events from Kafka. This is usually caused by " +
        ...
        logger.info("=KafkaTransaction=>eventTaken =  true");
        eventTaken = true;
        events.get().add(e);

      }catch (Exception ex){
        logger.error("=KafkaTransaction=>doTake error",ex);
      }
      logger.info("=KafkaTransaction=>doTake end");
      return e;
    }

    @Override
    protected void doCommit() throws InterruptedException {
      logger.info("=KafkaTransaction=> doCommit() begin");
      try {
        if (type.equals(TransactionType.NONE)) {
          logger.info("=KafkaTransaction=> doCommit() end type.equals(TransactionType.NONE)");
          return;
        }
        if (type.equals(TransactionType.PUT)) {
          logger.info("=KafkaTransaction=> doCommit() type.equals(TransactionType.PUT)");
          ...
          try {
            ...
            logger.info("=KafkaTransaction=> doCommit() batchSize"+batchSize);
            long startTime = System.nanoTime();
            int index = 0;
            ...
            logger.info("=KafkaTransaction=> doCommit() kafkaFutures："+JSON.toString(kafkaFutures));
            ...
            logger.error("=KafkaTransaction=>Sending events to Kafka failed", ex);
            throw new ChannelException("Commit failed as send to Kafka failed",
                    ex);
          }
        } else {
          logger.info("=KafkaTransaction=> doCommit() consumerAndRecords.get().commitOffsets() begin consumerAndRecords.get().failedEvents.isEmpty()"+consumerAndRecords.get().failedEvents.size()+" eventTaken:"+eventTaken);
          if (consumerAndRecords.get().failedEvents.isEmpty() && eventTaken) {
            logger.info("=KafkaTransaction=> doCommit() consumerAndRecords.get().commitOffsets() real begin");
            long startTime = System.nanoTime();
//            consumerAndRecords.get().commitOffsets();
        ...
        logger.error("=KafkaTransaction=>doCommit error",ex);
      }
      logger.info("=KafkaTransaction=> doCommit() end");
    }

    @Override
    protected void doRollback() throws InterruptedException {
      logger.info("=KafkaTransaction=> doRollback() begin");

      try {
        if (type.equals(TransactionType.NONE)) {
          logger.info("=KafkaTransaction=> doRollback() end type.equals(TransactionType.NONE)");
          return;
        }
        if (type.equals(TransactionType.PUT)) {
          producerRecords.get().clear();
      ...
      logger.error("=KafkaTransaction=>doRollback error",ex);
    }
      logger.info("=KafkaTransaction=> doRollback() end");
    }
  ...
  private class ConsumerAndRecords {
    ...
    void commitOffsets() {
      logger.info("=commitOffsets=> offsets"+JSON.toString(offsets));
//      this.consumer.commitSync(offsets);
    ...
    public void printCurrentAssignment() {
      logger.info("=printCurrentAssignment=>printCurrentAssignment begin");
      ...
      for (TopicPartition tp : this.consumer.assignment()) {
        try {
          ...
          logger.error("=printCurrentAssignment=>Committed ", npe);
          if (logger.isDebugEnabled()) {
            logger.debug("Committed {}", tp);
          }
        }catch (Exception ex){
          logger.error("=printCurrentAssignment=>Committed ", ex);
        }
      }
      logger.info("=ConsumerAndRecords=>printCurrentAssignment end");
    }
  }
}
...
class ChannelCallback implements Callback {
  ...
  public void onCompletion(RecordMetadata metadata, Exception exception) {
    log.info("=ChannelCallback=>onCompletion begin");
    ...
    log.info("=ChannelCallback=>onCompletion end");
  }
}
...
class ChannelRebalanceListener implements ConsumerRebalanceListener {
  ...
  public void onPartitionsRevoked(Collection<TopicPartition> partitions) {
    log.info("=ChannelRebalanceListener=>onPartitionsRevoked begin");
    for (TopicPartition partition : partitions) {
      log.info("topic {} - partition {} revoked.", partition.topic(), partition.partition());
      rebalanceFlag.set(true);
    }
    log.info("=ChannelRebalanceListener=>onPartitionsRevoked end");
  }

  public void onPartitionsAssigned(Collection<TopicPartition> partitions) {
    log.info("=ChannelRebalanceListener=>onPartitionsAssigned begin");
    for (TopicPartition partition : partitions) {
      log.info("topic {} - partition {} assigned.", partition.topic(), partition.partition());
    }
    log.info("=ChannelRebalanceListener=>onPartitionsAssigned end");
  }
}
